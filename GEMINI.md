...
1. **Архитектура 3 сервера (КРИТИЧЕСКИ ВАЖНО):**
   - `37.1.212.51` — **Панель + Бот** (подписки, управление, проекты)
   - `38.180.81.181` — **Только VPN** (Xray Core, Blue-Green)
   - `185.4.67.223` — **РФ-релей** (мост, dokodemo подписки → панель)
2. **Reality SNI:** `dzen.ru` на US-сервере, `ozon.ru` на релее
3. **UUID сервера:** `eb4a1cf2-4235-4b0a-83b2-0e5a298389ed`
4. **При перезапуске Xray на US** — перезапустить `vpn-panel` на `37.1.212.51`
5. **При перезапуске Xray на релее** — перезапустить SSH-туннели на обоих серверах + `vpn-panel`
6. **При диагностике VPN** — использовать команды из `network_diagnostics.md`
7. **При проблемах с блокировками** — использовать `censorship_evasion.md`
8. **Happ подписка** — строго 6 протоколов: 3 от Ветки 1 (Relay RU new, gRPC Relay RU new, Vision) + 3 от Ветки 2 (Relay RU 2, gRPC Relay RU 2, Vision 2)
9. **Режим диалога (КРИТИЧЕСКИ ВАЖНО):** Сначала агент задает вопросы, изучает видение пользователя, и только после согласования приступает к изменению кода.
10. **Лог инцидентов (ОБЯЗАТЕЛЬНО):** После любого исправления — добавить запись в `docs/INCIDENT_LOG.md`. Перед диагностикой — **СНАЧАЛА проверить INCIDENT_LOG.md**.
11. **Панель на VPN-сервере ЗАПРЕЩЕНА.** Панель работает ТОЛЬКО на `37.1.212.51`.
12. **Health-check WARP:** ТОЛЬКО через `cloudflare.com/cdn-cgi/trace` (HTTP 200 + `warp=on`). Google, OpenAI и прочие крупные сервисы банят shared WARP IP.
13. **Параллельные Blue-Green ветки (КРИТИЧЕСКИ ВАЖНО):**
    - Ветка 1 (порты 443, 2053) — основная, НЕ трогать при доработках.
    - Ветка 2 (порты 10443, 12053) — резервная/тестовая, все доработки здесь.
    - Ветка 1 дорабатывается ТОЛЬКО когда Ветка 2 стабильно работает.
    - **НИКОГДА не менять обе ветки одновременно.**
14. **ID серверов:** VPN `38.180.81.181` = **433343**, Релей `185.4.67.223` = **433815**, Панель `37.1.212.51` = **351340**
15. **Регламент AI-агентов:** Никаких домыслов — всегда искать решения в сети. Строго следовать алгоритму: Найти причину → Требования к решению → Исследовать сеть → Найти решения → Внедрить → Протестировать. Использовать скиллы проекта, в том числе Loop Engineering.
