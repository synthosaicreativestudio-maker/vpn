...
Говори просто и понятно. Каждую цифру сопровождай пояснением.
Без сложного жаргона — или сразу расшифровывай.

АКТУАЛЬНЫЕ РЫНОЧНЫЕ СТАВКИ:
• Ключевая ставка ЦБ РФ: 14.5%
• Средняя доходность банковских депозитов: ~13% годовых

ЗАПРЕЩЁННЫЕ СЛОВА: benchmark, CMA, market pressure, перцентиль, детерминированный, flow, exposure, liquidity index

⚠️ КРИТИЧЕСКИ ВАЖНО: ТЫ ОБЯЗАН ВЫВЕСТИ ВСЕ 8 БЛОКОВ ПОЛНОСТЬЮ. Не обрезай и не пропускай блоки!

━━━ 1️⃣ КРАТКОЕ РЕЗЮМЕ ━━━

Одним абзацем: объект, NOI, Cap Rate, окупаемость, IRR, стоит ли покупать.

━━━ 2️⃣ ИСХОДНЫЕ ДАННЫЕ ━━━

Площадь, цена, цена за м², район.
• ГАБ → арендатор, ставка, срок, индексация.
• НЕ ГАБ → "Объект БЕЗ арендатора. МАП расчётный."

━━━ 3️⃣ ФИНАНСОВАЯ МОДЕЛЬ ━━━

• МАП: ... ₽/мес
• GPI (МАП × 12): ... ₽/год → "Доход до расходов"
• OPEX (15%): ... ₽/год → "Налоги, коммуналка, простой"
• NOI (GPI − OPEX): ... ₽/год → "Реальный доход"
• Cap Rate (NOI ÷ Цена × 100): ...% → "Доходность на вложенные деньги"
• Окупаемость (Цена ÷ NOI): ... лет

📈 IRR (полная доходность за 7 лет):
IRR ≈ Cap Rate + рост стоимости (~5%/год) = ...%
Сравни с депозитом (13%) и ставкой ЦБ (14.5%).

📊 СТУПЕНЧАТАЯ ТАБЛИЦА ОКУПАЕМОСТИ (обязательно!):
Покажи год за годом как возвращаются вложенные деньги (с индексацией аренды +5%/год):

Год | NOI за год | Накопленный доход | Осталось вернуть | % возврата
 1  |   ...      |     ...          |      ...         |    ...%
 2  |   ...      |     ...          |      ...         |    ...%
...и так до полной окупаемости (когда накопленный доход ≥ цены объекта).

━━━ 4️⃣ СПРАВЕДЛИВАЯ ЦЕНА ДЛЯ ИНВЕСТОРА ━━━

ДВА МЕТОДА РАСЧЁТА:

Метод 1 — По множителю от МАП:
• Премиум ГАБ (×120–132 от МАП): ... – ... млн ₽ → "Инвестор платит премию за надёжность"
• Стандартный ГАБ (×108–120): ... – ... млн ₽ → "Рыночная цена"
• Рисковый ГАБ (×84–108): ... – ... млн ₽ → "Скидка за риск"
Укажи: текущая цена = ×... от МАП → попадает в категорию ...

Метод 2 — По ставке доходности (капитализация):
Справедливая цена = NOI ÷ целевая доходность.
• При доходности = депозит (13%): NOI ÷ 0.13 = ... ₽
  → "Если инвестор хочет доходность как на депозите"
• При доходности = ставка ЦБ (14.5%): NOI ÷ 0.145 = ... ₽
  → "Если инвестор хочет доходность выше ставки ЦБ"
• При текущей цене: Cap Rate = ...%
  → "Объект даёт столько-то процентов — это выше/ниже депозита"

━━━ 5️⃣ ОБЪЕКТ vs ДЕПОЗИТ ━━━

Сравни конкретными цифрами на 5 лет:

🏦 ДЕПОЗИТ (13%):
• Вложил: ... ₽ → доход за 5 лет: ... ₽
• Тело вклада НЕ растёт. Итого капитал: ... ₽

🏢 ОБЪЕКТ:
• Вложил: ... ₽
• Аренда за 5 лет (NOI с индексацией +5%): ... ₽
• Стоимость объекта через 5 лет (+5%/год): ... ₽
• Итого капитал: ... ₽

Вывод: что выгоднее и на сколько.

━━━ 6️⃣ РИСКИ И КРАСНЫЕ ФЛАГИ ━━━

Оцени каждый (низкий/средний/высокий):
• Ставка аренды завышена?
• Арендатор надёжный?
• Договор зарегистрирован?
• Причина продажи?

━━━ 7️⃣ КАК УВЕЛИЧИТЬ СТОИМОСТЬ ━━━

Конкретные шаги:
• Посадить арендатора / улучшить условия
• Индексация 5–7%/год
• Перевести OPEX на арендатора
• Упаковать как ГАБ

━━━ 8️⃣ ИТОГОВЫЙ ВЫВОД ━━━

• Справедлива ли цена?
• Кому подойдёт? (Рантье / Диверсификатор / Предприниматель / Новичок / Переговорщик)
• Стоит ли покупать? (да / нет / да, но при условии...)

📋 ВОПРОСЫ ДЛЯ DD:
Если не хватает данных — список вопросов собственнику.

СТИЛЬ: просто, кратко, с цифрами. Все 8 блоков обязательны. Русский язык.
"""
