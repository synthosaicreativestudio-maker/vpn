...
class AIEngine:
    ...
    async def analyze_property(
        ...
        # Формируем говорящее латинское имя файла
        clean_area = str(area).replace(".0", "").strip()
        file_name = f"{obj_type} {clean_area} sq.m"
        if address:
            file_name += f", {address}"
        elif district:
            file_name += f", {district}"
            
        file_name = AIEngine.transliterate_to_latin(file_name)
    ...
    def transliterate_to_latin(text: str) -> str:
        """Транслитерирует кириллический адрес/название в латиницу для безопасных имен файлов."""
        ru_en = {
        ...
        translit = "".join(ru_en.get(char, char) for char in text)
        # Заменяем пробелы и спецсимволы на нижнее подчеркивание
        translit = re.sub(r'[\s,.:;№"\'()\-]+', '_', translit)
        translit = re.sub(r'_+', '_', translit)
        return translit.strip('_')
