...
def build_investment_report(
    *,
    ...
    if is_gab:
        # Вариант А для существующего ГАБ: оптимизация условий (индексация и т.д.)
        capex_a = 0.0
        delta_map_a = max(0.0, market.map_value - decision_base.map_value)
        payback_capex_a = "сразу"
    else:
    ...
    if is_gab:
        lines.append("<b>2. Сводное сравнение стратегий оптимизации</b>")
        lines.append("Показатель | Текущее состояние | Вариант А: Оптимизация ГАБ | Вариант Б: Редевелопмент")
    else:
    ...
    # === ЧАСТЬ 2. ВАРИАНТЫ ПЕРЕУПАКОВКИ ОБЪЕКТА ДЛЯ СОБСТВЕННИКА / СТРАТЕГИИ ПОВЫШЕНИЯ ДОХОДНОСТИ ===
    if is_gab:
        lines.append("<b>ЧАСТЬ 2. СТРАТЕГИИ ПОВЫШЕНИЯ ДОХОДНОСТИ СУЩЕСТВУЮЩЕГО БИЗНЕСА</b>")
        lines.append("Для оптимизации текущих показателей арендного бизнеса и увеличения рыночной стоимости объекта рекомендуется рассмотреть два сценария:")
        lines.append("")
        lines.append("<b>14. Вариант А: Оптимизация текущего ГАБ</b>")
        lines.append("Суть: индексация арендных ставок, пересмотр условий текущего договора (перевод коммунальных и эксплуатационных расходов на арендатора), аудит и фиксация долгосрочных отношений.")
        lines.append(f"- Целевая рыночная ставка аренды: {_rate(market_rate)}")
        lines.append(f"- Ожидаемый ежемесячный доход (МАП) после оптимизации: {_money(market.map_value)}")
        lines.append(f"- Окупаемость объекта по текущей цене снизится до {_years(market.payback)} (вместо {_years(decision_base.payback)}).")
        lines.append(f"- Ценность объекта как ГАБ для инвесторов вырастет до {_money(market.noi * 9)} (окупаемость 9 лет).")
        lines.append(f"- Необходимые вложения (CAPEX на оптимизацию): {_money(capex_a)}")
        lines.append(f"- Срок окупаемости этих вложений за счет прироста аренды: {payback_capex_a}")
        lines.append("")
        lines.append("<b>15. Вариант Б: Редевелопмент при ротации арендатора</b>")
        lines.append("Суть: при смене текущего арендатора — нарезка помещения на 3-4 независимых мелких блока. Ставка на мелкий формат всегда выше среднерыночной на 35%. Это также минимизирует риски простоя.")
