...
async def _get_live_connections() -> dict:
    """Собирает live TCP-соединения Xray с US (локально) и Relay (SSH).

    Возвращает dict с ключами:
      - connections: list[dict] — IP, count, email, port, server, send_q
      - summary: dict — total_connections, unique_ips, time_wait, us/relay counts
    """
    import re as _re
    from collections import Counter

    # Паттерн для парсинга ss output:
    # ESTAB 0 5350 10.129.0.4:49212 37.1.212.51:2053 users:(("xray",...))
    # ESTAB 0 0 [::ffff:10.129.0.4]:8443 [::ffff:85.140.4.70]:51277 users:((...))
    ss_line_re = _re.compile(
        r"ESTAB\s+\d+\s+(?P<sendq>\d+)\s+"
        r"(?P<local>\S+)\s+"
        r"(?P<remote>\S+)\s+"
        r"users:\(\("
    )

    def _parse_ss_output(text: str, server_label: str) -> list[dict]:
        """Парсит вывод ss -tnp, возвращает список записей."""
        results = []
        for line in text.strip().split("\n"):
            m = ss_line_re.search(line)
            if not m:
                continue
            remote_raw = m.group("remote")
            send_q = int(m.group("sendq"))
            local_raw = m.group("local")

            # Извлечь remote IP (убираем [::ffff:] обёртку и порт)
            remote_clean = remote_raw.replace("[::ffff:", "").rstrip("]")
            # Формат: IP:PORT или IP]:PORT
            if "]:" in remote_clean:
                remote_ip = remote_clean.split("]:")[0]
            elif remote_clean.count(":") == 1:
                remote_ip = remote_clean.rsplit(":", 1)[0]
            else:
                remote_ip = remote_clean

            # Извлечь local port
            local_clean = local_raw.replace("[::ffff:", "").rstrip("]")
            if "]:" in local_clean:
                local_port = local_clean.split("]:")[-1]
            elif local_clean.count(":") == 1:
                local_port = local_clean.rsplit(":", 1)[-1]
            else:
                local_port = "?"

            # Пропускаем loopback и SSH
            if remote_ip in ("127.0.0.1", "::1") or local_port == "22":
                continue
    ...
    try:
        ...
        if stdout:
            lines = stdout.decode("utf-8", errors="ignore")
            # Фильтруем только xray
            xray_lines = "\n".join(line for line in lines.split("\n") if "xray" in line)
            all_entries.extend(_parse_ss_output(xray_lines, "US"))
            # TIME-WAIT count
            time_wait_us = lines.count("TIME-WAIT")
    except Exception as e:
        logger.warning("Failed to get US connections: %s", e)

    # 2. Relay (через SSH)
    try:
        from panel.config import RELAY_ENABLED, RELAY_IP
        if RELAY_ENABLED or RELAY_IP:
            proc = await asyncio.create_subprocess_exec(
                "ssh", "-o", "StrictHostKeyChecking=no",
                "-o", "ConnectTimeout=5",
                f"ubuntu@{RELAY_IP}",
                "sudo ss -tnp && echo '---TIMEWAIT---' && sudo ss -tn state time-wait | wc -l",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.DEVNULL,
            )
            try:
            ...
            if stdout:
                ...
                parts = output.split("---TIMEWAIT---")
                xray_lines = "\n".join(
                    line for line in parts[0].split("\n") if "xray" in line
                )
                all_entries.extend(_parse_ss_output(xray_lines, "Relay"))
                if len(parts) > 1:
                    try:
                        time_wait_relay = int(parts[1].strip())
                    except ValueError:
                        pass
    except Exception as e:
        logger.warning("Failed to get Relay connections: %s", e)

    # 3. Группировка по IP
    ip_counter: Counter = Counter()
    ip_ports: dict[str, set[str]] = {}
    ip_server: dict[str, str] = {}
    ip_send_q: dict[str, int] = {}
    for entry in all_entries:
        ip = entry["ip"]
        ip_counter[ip] += 1
        ip_ports.setdefault(ip, set()).add(entry["port"])
        ip_server[ip] = entry["server"]
        ip_send_q[ip] = ip_send_q.get(ip, 0) + entry["send_q"]

    # 4. Привязка IP → email через ip_log
    ip_emails: dict[str, str] = {}
    try:
        with db._get_conn() as conn:
            rows = conn.execute(
                "SELECT ip, email, MAX(last_seen) as last_seen "
                "FROM ip_log GROUP BY ip ORDER BY last_seen DESC"
            ).fetchall()
            for row in rows:
                ip_emails[row["ip"]] = row["email"]
    except Exception:
        pass
    ...
    for ip, count in ip_counter.most_common():
        connections.append({
            "ip": ip,
            "count": count,
            "email": ip_emails.get(ip, "—"),
            "ports": sorted(ip_ports.get(ip, set())),
            "server": ip_server.get(ip, "?"),
            "send_q": ip_send_q.get(ip, 0),
        })

    us_count = sum(1 for e in all_entries if e["server"] == "US")
    relay_count = sum(1 for e in all_entries if e["server"] == "Relay")

    return {
        "connections": connections,
        "summary": {
            "total_connections": len(all_entries),
            "unique_ips": len(ip_counter),
            "time_wait": time_wait_us + time_wait_relay,
            "us_connections": us_count,
...
@app.get(
    ...
    dependencies=[Depends(verify_api_key)],
    tags=["Система"],
    summary="Live TCP-соединения Xray (US + Relay)",
)
async def admin_connections():
    """Возвращает текущие TCP-соединения Xray, сгруппированные по IP."""
    return await _get_live_connections()
