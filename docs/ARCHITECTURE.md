...
## 📊 Управление логами и предотвращение переполнения диска

Для исключения сбоев из-за нехватки свободного места на серверах установлена жесткая политика логирования:

### 1. Xray Logging
- **`access.log` на production полностью отключен** (`"access": "none"` в `config.json`). Xray записывает каждое соединение, что генерирует гигабайты логов.
- **`error.log` включен на уровне предупреждений** (`"loglevel": "warning"`). Это позволяет видеть ошибки конфигурации и TLS без засорения диска.

### 2. Logrotate для Xray
На VPN-сервере настроен `/etc/logrotate.d/xray` со следующими параметрами:
```logrotate
/var/log/xray/*.log {
    size 50M
    rotate 3
...
```
*Примечание: используется `copytruncate`, так как Xray держит дескрипторы открытыми и не требует перезапуска при ротации.*

### 3. Ограничение Systemd Journald
На всех серверах в `/etc/systemd/journald.conf` задан лимит:
```ini
SystemMaxUse=100M
```
Это гарантирует, что системный журнал не займет более 100 MB дискового пространства.
