...
## 2026-07-18 (4) — Переход подписок на чистый gRPC для обеих ветвей

### Задача
- Пользователь запросил оставить в подписках пользователей исключительно протоколы gRPC для Ветки 1 (порт 2053 через релей) и Ветки 2 (порт 12053 через релей) и удалить все остальные протоколы (Vision Direct, Vision Relay, xHTTP, WS, Hysteria2, Shadowsocks).

### Решение
1. В `panel/models.py` добавлены новые поля `vless_relay_grpc_1` и `vless_relay_grpc_2` в схему `SubscriptionLinks`.
2. В `panel/link_generator.py` переписаны методы генерации ссылок (`all_links`, `hiddify_links`, `happ_links`, `happ_test_links`) так, чтобы они возвращали словари только с gRPC-ссылками.
3. В `panel/templates/dashboard.html` обновлена секция ручных настроек "Отдельные протоколы" для отображения только актуальных gRPC-подключений.
4. Проведена локальная верификация, линтинг `ruff check .` пройден.
5. Изменения задеплоены на сервер `37.1.212.51`, службы `vpn-panel` и `vpn-bot` перезапущены.
6. В `docs/ARCHITECTURE.md` актуализировано описание подписок.

### Проверка
- [x] Локальный `/health` на сервере панели → `online`, `xray_connected: true`.
- [x] GET-запрос к API `/users/{email}/links` отдает пустые значения для старых протоколов и корректные URIs для `vless_relay_grpc_1` и `vless_relay_grpc_2`.
- [x] Декодированные подписки (`/sub/happ/`, `/sub/happ-android/`, `/sub/`) выдают ровно две gRPC-ссылки.
...
### Симптомы
- VPN-сервер `38.180.81.181`: диск заполнен на 99% (312 MB свободно из 20 GB).
- `/var/log/xray/` занимал **5.8 GB** (access.log с записью каждого пакета).
- `/var/log/journal/` занимал **1.2 GB** (без лимита).
- Мусорные директории `/root/vpn-blue/`, `/root/vpn-green/`, `/root/vpn/` занимали **~4.2 GB** (остатки старых деплоев панели/бота, которые на VPN-сервере не используются).

### Диагностика
1. **Корневая причина #1:** Xray access.log записывает **каждое соединение каждого клиента** (DNS, TLS handshake, routing decisions) в файл, даже при `loglevel: warning`. Запись `access` была включена (`"/var/log/xray/access.log"`), а ротация отсутствовала.
2. **Корневая причина #2:** Файл `/etc/logrotate.d/xray` на VPN-сервере **отсутствовал** — логи никогда не архивировались и не удалялись.
3. **Корневая причина #3:** В `/etc/systemd/journald.conf` параметр `SystemMaxUse` был закомментирован (дефолт Ubuntu: до 10% диска).
4. **Мусор:** Директории `/root/vpn*` содержали копии панели и бота (по архитектуре они работают ТОЛЬКО на `37.1.212.51`), включая тяжёлые venv RBN-бота (~1 GB каждый).

### Решение (Loop Engineering: Discover → Plan → Execute → Verify)
| # | Действие | Детали |
|---|----------|--------|
| 1 | **Очистка access.log** | `truncate -s 0 /var/log/xray/access.log` — освободило ~5.8 GB |
| 2 | **Удаление мусора** | `rm -rf /root/vpn-blue /root/vpn-green /root/vpn` — освободило ~4.2 GB |
| 3 | **Отключение access.log в Xray** | `"access": "none"` в config.json (на VPN-сервере и релее). Error.log оставлен для отлова реальных проблем |
| 4 | **Создание logrotate** | `/etc/logrotate.d/xray`: ротация при >50 MB, 3 файла, сжатие, `copytruncate` |
| 5 | **Лимит journald** | `SystemMaxUse=100M` в `/etc/systemd/journald.conf`, перезапуск `systemd-journald` |
| 6 | **Обновление stable** | `config.json.stable` обновлён на VPN-сервере и релее |
| 7 | **Перезапуск Xray** | На VPN-сервере и релее, с последующим перезапуском SSH-туннелей и панели |

### Верификация
- [x] Диск VPN-сервера: с 99% (312 MB) → **41% (12 GB свободно)**
- [x] Ветка 1 (443, 2053): порты слушают, TLS handshake OK (dzen.ru)
- [x] Ветка 2 (10443, 12053): порты слушают, TLS handshake OK
- [x] WARP: `Connected`, `healthy`
- [x] Panel health: `xray_connected: true`
- [x] Failover timer: `active`
- [x] Logrotate: dry-run чистый, без предупреждений
- [x] Journald: лимит 100M применён (текущий 72M)
- [x] Stable config: `access: "none"`, `loglevel: "warning"`
- [x] Релей: Xray active, обе ветки работают, `access: "none"`

### Урок
> **На production VPN-серверах access.log Xray должен быть отключён (`"access": "none"`)** — он записывает каждое соединение каждого клиента и быстро заполняет диск. Error.log (`"loglevel": "warning"`) достаточен для диагностики реальных проблем. **Обязательно настраивать logrotate** для всех лог-файлов при первоначальной установке сервиса. **Обязательно ограничивать journald** через `SystemMaxUse=100M`.
